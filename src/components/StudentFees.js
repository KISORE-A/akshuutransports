...
export default function StudentFees() {
  ...
  const totalFees = profile.totalFees || 0;
  const paidFees = profile.paidFees || 0;
  const due = Math.max(0, totalFees - paidFees);
  const percent = totalFees > 0 ? Math.round((paidFees / totalFees) * 100) : 0;

  return (
    <div className="card" style={{ padding: "1.5rem" }}>
      <h3 style={{ color: "#2b3674", marginTop: 0 }}>💰 Fee Status</h3>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "12px" }}>
        <div style={{ background: "#f8fafc", padding: "12px", borderRadius: "10px" }}>
          ...
          <div style={{ fontSize: "1.2rem", fontWeight: 800, color: "#2b3674" }}>₹{totalFees.toLocaleString()}</div>
        </div>
        <div style={{ background: "#f0fff4", padding: "12px", borderRadius: "10px" }}>
          <div style={{ fontSize: "0.75rem", color: "#2f855a" }}>Paid</div>
          <div style={{ fontSize: "1.2rem", fontWeight: 800, color: "#2f855a" }}>₹{paidFees.toLocaleString()}</div>
        </div>
        <div style={{ background: "#fff5f5", padding: "12px", borderRadius: "10px" }}>
          <div style={{ fontSize: "0.75rem", color: "#e53e3e" }}>Due</div>
          <div style={{ fontSize: "1.2rem", fontWeight: 800, color: "#e53e3e" }}>₹{due.toLocaleString()}</div>
        </div>
      </div>
      <div style={{ marginTop: "14px" }}>
        ...
        <div style={{ height: "8px", background: "#edf2f7", borderRadius: "999px" }}>
          <div style={{ height: "100%", width: `${percent}%`, background: "linear-gradient(90deg,#4318FF,#05cd99)", borderRadius: "999px" }} />
        </div>
